...
import com.adidas.subscription.exceptions.customs.ParseException;
...
public class Subscription {
    ...
    public Subscription(String email, String firstName, String gender, String dateOfBirth, Boolean subscriptionConsent, Integer campaignId) {
        if(emailNotMatches(email)){
            log.error("Invalid email {}", email);
            throw new InvalidRequestException("Invalid email address");
        }

        if(dateNotMatches(dateOfBirth)){
            log.error("Invalid date of birth {}", dateOfBirth);
            throw new InvalidRequestException("Invalid date of birth");
        }

        if(subscriptionConsent == null){
            log.error("Consent is null");
            throw new InvalidRequestException("Must answer newsletter terms");
        }

        if(campaignId == null){
            log.error("CampaignId is null");
            throw new InvalidRequestException("Must inform campaign id");
        }
        this.enabled = true;
        this.email = email;
        this.firstName = firstName;
        this.gender = GenderEnum.getGenderByDisplayName(gender).get();
        try{
    ...
    private boolean emailNotMatches(String email){
       return !VALID_EMAIL_ADDRESS.matcher(email).matches();
    }

    private boolean dateNotMatches(String date){
